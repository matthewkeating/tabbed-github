...
**URLs from the system.** `src-tauri/Info.plist` (merged into both bundles by the Tauri bundler) declares `CFBundleURLTypes` for `http`/`https` at `LSHandlerRank: Alternate`, which is what makes the app a legal target for URL routers such as Velja — without the declaration LaunchServices refuses to hand it URLs at all. `Alternate` says "can open these", not "I am the default browser"; the app still shows up in Open With menus and the default-browser list, which is unavoidable. macOS delivers these via `application:openURLs:`, which Tauri surfaces as `RunEvent::Opened` — hence `run()` uses `.build(...).run(|app, event| ...)` rather than the plain `.run(context)`, and `open_url_from_system` routes each URL through the same `is_in_app_host` rule as an in-page link. No `tauri-plugin-deep-link` needed. **This is a third entry point into the app** alongside the menu and global shortcuts.

**`RunEvent::Opened` races `setup`, in both directions.** A URL that launches the app is delivered before the `setup` hook runs about as often as after — this was observed flipping between runs of the same build, so do not assume either order. Both sides are handled, and a change to one needs the other: `setup` creates its `start_url` tab only `if app.webview_windows().is_empty()` (a URL got there first and already opened the tab), while `startup_tab_to_reuse` navigates the existing startup tab instead of opening a second one (`setup` got there first). Without both halves a cold launch from a URL strands a stray `start_url` tab next to the requested page. The reuse is one-shot and time-boxed (`STARTUP_TAB_REUSE_WINDOW`) so ordinary URLs, arriving long after launch, always get their own tab.
...
- **URL handling only exists in a bundled build.** `tauri dev` produces no `.app` and therefore no `Info.plist`, so `RunEvent::Opened` never fires — test URL routing against a `pnpm deploy`'d build. LaunchServices may also need `lsregister -f ~/Applications/GitHub.app` to notice a changed declaration. Note the build output under `target/release/bundle/macos/` registers as an http(s) handler too, so URL routers can show two "GitHub" entries; the `~/Applications` copy is the real one.
